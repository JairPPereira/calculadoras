...
*/
  ...
  if (prom[0].toString() != "") {  // zadano alfa     
    ...
    prom[1] = Math.sin(prom[0]);
    prom[2] = 0;
    postupovka = 0;
  } else {  // zadano sin alfa   
    ...
    if ((prom[1] == 0)||(Math.abs(prom[1]) == 1)) {         
      ...
      if (prom[0] > 0) {
        prom[2] = Math.PI - prom[0];
      } else {   // prom[0] < 0       
        prom[2] = prom[0];
        prom[0] = 2 * Math.PI - (prom[2] - Math.PI);
      }            
      Vypiska900(2, 1);  
      postupovka = 3;
    }   
...
function Graf(postupovka, prom0, prom1, prom2) {     
  ...
  if (postupovka == 1) {
    vypis[4] = "<polyline points=\"70,175 430,175\" class=\"s_blue strok\" />";
    var popisX1 = [];
    for (i=1; i<3; i++) {   
      if (document.getElementById("jed0").selectedIndex == 0) {
        popisX1[i-1] =  ((i * 180).toFixed(document.getElementById("desMista1").value)).toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, " ").replace(".", ",") + "&nbsp;&deg;";
      } else {
        popisX1[i-1] =  ((i * Math.PI).toFixed(document.getElementById("desMista1").value)).toString().replace(/\B(?=(?:\d{3})+(?!\d))/g, " ").replace(".", ",") + "&nbsp;" + document.getElementById("jed" +  (i - 1) * 2).options[1].innerHTML;
      }
     vypis[i + 4] = "<circle cx=\"" + (sourX[0] + i * 180) + "\" cy=\"" + sourY + "\" r=\"2\" class=\"f_blue\" />";
     vypis[i + 6] = "<text x=\"" + (sourX[0] + i * 180) + "\" y=\"" + popY + "\" class=\"smallerreg start\" style=\"writing-mode:tb;\">" + popisX1[i-1] + "</text>";
    }
  }
  if (postupovka == 3) {
