function Kalkul() {
  /* PROMENNE - sestiuhelnik
  ==========================
     prom0      strana, polomer kruznice vepsane
     prom1      polomer kruznice opsane       
  ...
  */
  ...
  if (prom[0] != "") {  // strana
    ...
    stranaM = 2 * prom[2]/Math.sqrt(3);
    latex = 2;
  } else if (prom[3] != "") {  //obsah
    stranaM = Math.sqrt(2 * prom[3]/(3 * Math.sqrt(3)));
    latex = 3;
  ...
  var polomerkvM = Math.sqrt(3)/2 * stranaM;
  var obsahM = 3 * Math.sqrt(3)/2 * Math.pow(stranaM,2);  
  var obvodM = 6 *stranaM; 
    
   // ulozeni promennych v metrech
   var promM = [stranaM, stranaM, polomerkvM, obsahM, obvodM];
...
function Latex() {
  if (document.getElementById("postupka").value != "") {
    ...
    document.getElementById("latex").src = "/aaa/php/postup/110/" + document.getElementById("postupka").value + ".php?jazverze=" + document.getElementById("jazverze").value + urlGet;
